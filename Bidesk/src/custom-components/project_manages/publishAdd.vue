<template>
  <el-row>
    ...
    <el-form v-if="pack_list === null" label-width="180px" class="demo-ruleForm"
      ...
      <el-row>
        <el-col :span="24">
          <packChecked @transferIdList="getIdList" @transferNum="getNumList"
                       :packNameList="packNameList" :packIdList="packIdList"
                       :current_tab="selectIndex" :packList="packList"
                       :pack_id_list="ruleForm.pack_id_list"></packChecked>
        </el-col>
      </el-row>
      <!--编辑框-->
      <el-row>
      ...
      <el-dialog title="提示" :visible.sync="dialogVisible">
        ...
        <el-row style="padding-top: 1%;">
          <el-checkbox class="checkbox__label" :checked="checked" v-model="checked" @change="checkedChange">
            <span style="color: #FF0000">是否确认发布采购文件,发布后将不可修改！</span>
          </el-checkbox>
        </el-row>
  ...
  export default {
    ...
    data() {
      return {
        ...
        must_click: true,
        packNameList: [this.packList[this.selectIndex].packNum],
    ...
    computed: {
      pack_list(){
        return this.packList[this.selectIndex].purchaseDoc
      },
      ...mapGetters([
        'name',
        'company',
    ...
    methods: {
      ...
      cancelSubmit(ruleForm) {
        this.packIdList = [this.packList[this.selectIndex].packId]
        this.packNameList = [this.packList[this.selectIndex].packNum]
      ...
      submitForm() {
        ...
        executeSaveAddNotice(this.packIdList, this.ruleForm).then(response => {
          ...
          if (response.state === true) {
            ...
            // 当前视图更新
            const id = response.data[this.packList[this.selectIndex].packId].packId
//          注意表单判断
            this.packList[this.selectIndex].purchaseDoc = response.data[id]
          } else {
            this.checked = false
            this.$message({
